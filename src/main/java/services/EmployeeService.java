...
public class EmployeeService extends ServiceBase {
     ...
     * @return 表示するデータのリスト
     */
    public List<EmployeeView> getPerPage(int page) { //指定されたページ数の一覧画面に表示するデータを取得
     ...
     */
    public long countAll() { //従業員テーブルのデータの件数を取得
     ...
     */
    public EmployeeView findOne(String code, String plainPass, String pepper) { //社員番号、パスワードを条件にデータを取得
        Employee e = null;
        try {
            //パスワードのハッシュ化
     ...
     * @return 取得データのインスタンス
     */
    public EmployeeView findOne(int id) { //idを条件にデータを1件取得
     ...
     * @return 該当するデータの件数
     */
    public long countByCode(String code) { //社員番号を条件に該当するデータの件数を取得します。重複した社員番号登録できないようチェックするときに使用
     ...
     */
    public List<String> create(EmployeeView ev, String pepper) { //画面入力された従業員の情報からデータを1件作成、バリデーション後、テーブルに登録
     ...
     */
    public List<String> update(EmployeeView ev, String pepper) { //画面入力された従業員の情報からデータを1件作成、バリデーション後、テーブルの該当データを更新
     ...
     * @param id
     */
    public void destroy(Integer id) { //idを条件に従業員データを論理削除（delete_flgを1に更新）
     ...
     */
    public Boolean validateLogin(String code, String plainPass, String pepper) { //社員番号とパスワードを条件に検索し、データが取得できるかどうかで認証結果を返却
     ...
     * @return 取得データのインスタンス
     */
    private Employee findOneInternal(int id) { //idを条件にデータを1件取得
     ...
     */
    private void create(EmployeeView ev) { //従業員データを1件登録
     ...
     * @param ev 画面から入力された従業員の登録内容
     */
    private void update(EmployeeView ev) { //従業員データを更新
